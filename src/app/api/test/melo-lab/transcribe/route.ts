...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (audio.size > MELO_LAB_MAX_AUDIO_BYTES) {
      return NextResponse.json(
    ...
    return NextResponse.json({
      ...
      stages: {
        ...
        intentProfile: {
          summary: summarizeMelodyIntent(result.melodyIntent),
        },
        intent: {
...
async function readFormData(request: NextRequest): Promise<FormData> {
  try {
    ...
    throw new AudioWorkerError(
      "validation_error",
      "Expected a multipart/form-data audio request.",
      400,
    );
  }
}
...
function summarizeMelodyIntent(
  ...
  return {
    ...
    tonalCandidates: profile.tonalCandidates,
    stableAnchorPitches: profile.stableAnchorPitches,
    phraseEndingPitches: profile.phraseEndingPitches,
    correctionPolicy: profile.correctionPolicy,
  };
}

function parsePitchProvider(value: FormDataEntryValue | null) {
  const raw = typeof value === "string" ? value.trim().toLowerCase() : "";
  if (!raw) return "auto";
  if (isMeloLabPitchProvider(raw)) {
    return raw;
  }
  throw new AudioWorkerError(
    "validation_error",
