...
import {
  MELO_LAB_MAX_AUDIO_BYTES,
  MELO_LAB_MAX_MUSIC_DURATION_SECONDS,
  MELO_LAB_MAX_PROMPT_CHARS,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (prompt.length > MELO_LAB_MAX_PROMPT_CHARS) {
      return NextResponse.json(
    ...
    if (hum && hum.size > MELO_LAB_MAX_AUDIO_BYTES) {
      return NextResponse.json(
    ...
    const workerUrl = workerBase.endsWith("/generate")
      ? workerBase
      : `${workerBase}/generate`;
    ...
    if (error instanceof MeloLabRequestError) {
      return NextResponse.json(
...
async function readFormData(request: NextRequest): Promise<FormData> {
  try {
    ...
    throw new MeloLabRequestError(
      "Expected a multipart/form-data music probe request.",
      400,
    );
  }
}

class MeloLabRequestError extends Error {
  constructor(
    message: string,
    public readonly status: number,
  ) {
    super(message);
    this.name = "MeloLabRequestError";
  }
}

function parseMelody(value: string): CleanMelody {
  if (!value) {
    throw new MeloLabRequestError("Melody JSON is required.", 400);
  }
  try {
    ...
    const message =
      error instanceof z.ZodError
        ...
        : "Melody must be valid JSON.";
    throw new MeloLabRequestError(message, 400);
  }
}
