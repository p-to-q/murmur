...
export function HumScreen() {
  ...
  const transcribeAndGenerate = async (blob: Blob | undefined) => {
    ...
    tickMessages();
    try {
      // Fire the Magenta health probe immediately so its (potentially slow,
      // cold-start) round-trip overlaps the transcription instead of being
      // paid serially after it — a timeout here silently demotes the whole
      // flow to the legacy engine, so give it every chance to resolve.
      const magentaReadyPromise = checkMusicEngineAvailable();
      ...
      // Magenta RealTime path: randomized prompt vibes, audio generated by
      // the local worker. Falls back to the legacy Tone.js arrangements when
      // the worker is unreachable (production without a worker, demo runs).
      const magentaReady = await magentaReadyPromise;
      setHumStyleBlob(magentaReady ? preparedBlob ?? null : null);
      const versions = magentaReady
