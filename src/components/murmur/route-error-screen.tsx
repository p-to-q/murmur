...
export interface RouteErrorScreenProps {
  ...
  reset: () => void;
  /** Log tag identifying which route boundary caught the error, e.g. "gallery". */
  scope: string;
}

// Shared body for every route-level error.tsx. Next.js requires a file per
// route segment, so each of those stays a thin wrapper around this screen.
export function RouteErrorScreen({ error, reset, scope }: RouteErrorScreenProps) {
