"""Auth hardening tests for the music-engine HTTP worker."""
...
class WorkerAuthRequiredTest(unittest.TestCase):
    ...
    def test_production_requires_token(self):
        with patch.dict(os.environ, {"NODE_ENV": "production"}, clear=True):
            ...
            with self.assertRaisesRegex(RuntimeError, "MUSIC_WORKER_TOKEN is required"):
                main.require_worker_token()

    def test_public_bind_requires_token(self):
        with patch.dict(os.environ, {"MUSIC_ENGINE_HOST": "0.0.0.0"}, clear=True):
            self.assertTrue(main.worker_auth_required())
            with self.assertRaisesRegex(RuntimeError, "MUSIC_WORKER_TOKEN is required"):
                main.require_worker_token()

    def test_explicit_require_auth_requires_token(self):
        with patch.dict(os.environ, {"MUSIC_WORKER_REQUIRE_AUTH": "1"}, clear=True):
            self.assertTrue(main.worker_auth_required())
            with self.assertRaisesRegex(RuntimeError, "MUSIC_WORKER_TOKEN is required"):
                main.require_worker_token()
    ...
    def test_token_satisfies_required_auth(self):
        with patch.dict(
            os.environ,
            {"NODE_ENV": "production", "MUSIC_WORKER_TOKEN": "tok_test"},
            clear=True,
        ):
            self.assertTrue(main.worker_auth_required())
            main.require_worker_token()
