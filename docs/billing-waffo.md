...
/api/billing/checkout
   ...
   │  • insert authoritative purchases(status = pending) snapshot
...
POST /api/billing/webhook
   ...
   │  • order.completed → find pending row by orderMerchantExternalId
   │  • validate user/SKU/amount/currency/notes against checkout snapshot
   │  • switch providerRef to final orderId + mark succeeded in transaction
...
- `metadata` — `{ userId, skuId, notesGranted, purchaseKind, customAmountUsd? }`.
  New sessions also include `pendingProviderRef`, matching the merchant order
  reference. Metadata is provider-signed context, but the `purchases` pending
  snapshot is the authoritative user/SKU/amount/currency/grant record.
...
- `orderMerchantExternalId` — `waffo-pending:pur_…`, generated by Murmur and
  temporarily stored as the pending purchase's `providerRef`.

Before calling Waffo, checkout inserts the pending purchase. After session
creation it stores the provider `sessionId` and expiry in `rawPayload`, then
returns `{ checkoutUrl, sessionId }`; the client opens `checkoutUrl`. A
provider-call failure can leave a pending row because the remote outcome may be
ambiguous; it grants nothing and is safe to inspect or expire later.
...
3. **Resolve** — `resolveWaffoTopupPurchase` flattens `orderMetadata`, converts
   the display amount to cents, and validates the fixed SKU or custom quote.
4. **Authorize** — for new sessions, require matching
   `orderMerchantExternalId` / `pendingProviderRef`, lock that pending purchase,
   and compare user, SKU, paid amount, currency, and notes against its stored
   checkout snapshot. Missing or mismatched snapshots are non-retryable and
   grant nothing.
5. **Finalize + grant** — in one DB transaction, change the row's
   `providerRef` from `waffo-pending:pur_…` to the final Waffo `orderId`, mark it
   `succeeded`, and insert the idempotent `purchase:topup` ledger grant keyed by
   `orderId`.
6. **Legacy compatibility** — an in-flight session created before authoritative
   pending rows has no `waffo-pending:` reference. It keeps the previous path:
   validate provider metadata/quote and insert a succeeded purchase by
   `orderId`. New-format references never use this fallback.
7. **Refund** — for `refund.succeeded`, find the local purchase by Waffo
...
New checkouts first reserve a unique `(provider, providerRef)` using the merchant
external reference; fulfillment switches that same row to the Waffo `orderId`
inside the grant transaction. The steady-state guards remain keyed on order id:

1. `events_webhook` unique `(provider, providerEventId)` — each Waffo event is
   processed once.
2. `purchases` unique `(provider, providerRef)` — each pending checkout and then
   each finalized order is represented once.
...
pins the session currency. New authoritative pending purchases therefore
require exact currency and cents equality with the stored snapshot. The legacy
in-flight fallback retains the older quote validation behavior, including the
converted-currency allowance, so deployment does not strand already-open
sessions.
...
final purchases by Waffo `orderId`, and also correlates unfulfilled pending rows
by `orderMerchantExternalId`. A succeeded payment that still maps to a pending
row is a hard `local_purchase_pending_after_payment` issue and is never
auto-granted: webhook replay must perform the atomic ref switch + grant. For
final rows it checks `purchases.amountCents` and the matching
`notes_ledger.reason = "purchase:topup"` row. It also reports
successful refunds that do not have an observed `refund:topup` ledger row in
the checked window. Refund matching checks the refund-ticket ref, webhook event
or provider refund id, final order id, and merchant order ref fallbacks. It
exits non-zero only for hard
