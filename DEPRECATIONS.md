...
|---|---|---|---|
| `src/modules/stainer/providers/browser-basic-pitch.ts` | Basic Pitch monophonic accuracy is too low for solo hum | server SwiftF0 | `docs/audio-pipeline-redesign.md` §4.3 |
| `src/modules/stainer/providers/remote-python.ts` | Superseded by the proxied `/api/transcribe` route | server-side audio worker | `docs/audio-pipeline-redesign.md` §6 |
...
| `src/app/api/transcribe/route.ts` proxy-only shape | Replaced by full pipeline + diagnostics | new route per `docs/audio-pipeline-redesign.md` §5 | same |
